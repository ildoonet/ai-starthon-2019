...
class SSD300(nn.Module):
    ...
    def detect_objects(self, predicted_locs, predicted_scores):
        ...
        for i in range(batch_size):
            ...
            decoded_locs = cxcy_to_xywh(gcxgcy_to_cxcy(predicted_locs[i], self.priors_cxcy))  # (8732, 4), these are fractional pt. coordinates

            # Check for each class
            c = 1
            # Keep only predicted boxes and scores where scores for this class are above the minimum score
            class_scores = predicted_scores[i][:, c]  # (8732)
            score_above_min_score = class_scores > 0.  # torch.uint8 (byte) tensor, for indexing
